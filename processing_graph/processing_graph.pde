...
int i=1;
float[] inByte= new float[40000]; //array of incoming bytes
void setup()
{
  size(400,300);//size of window
  println(Serial.list());//list available COM ports
 ...
 void serialEvent (Serial myPort)
  {
    String inString=myPort.readStringUntil('\n');//read ASCII string
      ...
      inByte[i]=map(inByte[i],0,1023,0,100);
      textSize(10);
      color(255);
      ...
      println(x1Pos + "," + y1Pos + "," + x2Pos + "," + y2Pos);
      
      line(xPos,inByte[i-1],++xPos,inByte[i++]);
